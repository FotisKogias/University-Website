...
// let monk = setToMonday(new Date());
// console.log(monk);
$("#existing2").change(function (event) {
  ...
  students.forEach((elem, index) => {
    ...
    $(`<div class="card col-md p-3 mb-5 ${elem.objectName.split(" ").join("")}">
        <div class="row mt-5 border-bottom ">
        <h2 class=" text-warning text-start ">${currentPage.charAt(0).toUpperCase() + currentPage.slice(1)}s due the above date:</h2>
        </div>
        <div class="row mt-5 text-center ">
    ...
    if (elem.assignments === undefined) {
      ...
      elem.assignments.forEach((element) => {
        ...
        let monday = setToMonday(new Date(array[find].subdate))
        let assignment_date = setToMonday(new Date($("#existing2").val()))
        if (Date.parse(monday) == Date.parse(assignment_date)) {
          console.log("nice");
          for (let i = 0; i < values.length; i++) {
